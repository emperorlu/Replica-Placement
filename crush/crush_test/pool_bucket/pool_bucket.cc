...
void pool_bucket::adjust_crushmap_pg_target(map<int, set<int>>& primary_change,
  ...
  float primary_per_weight = 1.0 * pool_primary_num / (root_weight - total_correct_weight);
  float duplicate_per_weight = 1.0 * pool_duplicate_num / (root_weight - total_correct_weight);
  primary_per_weight = 1.0 * pool_primary_num / (root_weight - total_correct_weight);
  duplicate_per_weight = 1.0 * (pool_duplicate_num) / (root_weight - total_correct_weight);
  for (auto& b : buckets_map) {
    b.second.primary_pg_num = ceil(b.second.correct_weight * primary_per_weight);
    b.second.duplicate_pg_num = ceil(b.second.correct_weight * duplicate_per_weight);
...
int pool_bucket::apply_map_change(map_change& change) {
  ...
  while (!virtual_nodes.empty() && valid_bucket_num + virtual_nodes.size() > pool_size) {
    int v_b_id = *virtual_nodes.rbegin();
    assert(buckets_map.count(v_b_id) && buckets_map[v_b_id].primary_pg.size() == 0 && buckets_map[v_b_id].duplicate_pg.size() == 0);
    buckets_map.erase(v_b_id);
    virtual_nodes.erase(v_b_id);
  }
...
void pool_bucket::correct_bucket_weight() {
  ...
  int start_virtual_id = virtual_nodes.empty() ? virtual_node_id : *(virtual_nodes.rbegin())+1; 
  for (auto& b : buckets_map) {
  ...
  valid_bucket_num = buckets_map.size() -  zero_bucket;
  if (valid_bucket_num < pool_size) { //can not select enough duplicate
    while (valid_bucket_num + virtual_nodes.size() < pool_size) { //need add new virtual_node
      virtual_nodes.insert(start_virtual_id);
      buckets_map[start_virtual_id] = bucket(start_virtual_id);
      start_virtual_id++;
    }
    for (auto& b : buckets_map) {
      if (b.second.weight || b.second.id >= virtual_node_id) { //valid_bucket or virtual_node
        b.second.correct_weight = 1;
      } else {
        b.second.correct_weight = 0;
      }
    }
    total_correct_weight = root_weight - (pool_size);
    auto iter = virtual_nodes.rbegin();
    while (valid_bucket_num + virtual_nodes.size() > pool_size) { //need add new virtual_node      
      assert(iter != virtual_nodes.rend());
      buckets_map[*iter].correct_weight = 0;
      iter++;
    }
    return;
  }
