...
int pool_pg_map::overfull_remap_step1(map<int, set<int>>& change_map, bool is_primary) {
  int remap_count = 0;
...
int pool_pg_map::overfull_remap_step2(map<int, set<int>>& change_map, bool is_primary) {
  int remap_count = 0;                                                                                     
            ...
            for (int pg : over_osd_info.primary_pg) {
              ...
              do {
                ...
                do {
                  swap = (swap+1)%max_osd;
                  find_swap_retry++;
                } while (find_swap_retry <= max_osd &&
                         (!osds_map.count(swap)
                            || osds_map[swap].primary_pg_num-osds_map[swap].primary_pg.size() > 0
                            || swap == over_osd));
                if(find_swap_retry > max_osd) {
                  break;
                }
                ...
                find_tmp_pg_retry++;
              } while (tmp_pg == invalid_id && find_tmp_pg_retry <= max_osd);
              if(tmp_pg == invalid_id) { //find swap fail
                to_remove.push_back(pg);
                pgs_to_osd[pg][0] = invalid_id;
                buckets_map[over_osd_info.bucket_id].primary_pg.erase(pg);
                continue;
              }
              ...
              do {
                ...
                do {
                  ...
                  find_swap_retry++;
                        ...
                        (!osds_map.count(swap)
                          || osds_map[swap].duplicate_pg_num-osds_map[swap].duplicate_pg.size() > 0
                          || swap == over_osd));
                if(find_swap_retry > max_osd) {
                  break;
                }
    ...
    if (is_primary) {
      ...
      do {
        ...
        do {
          ...
          find_osd_retry++;
        } while(find_osd_retry <= max_osd 
                && (!osds_map.count(balance_osd) 
                      || osds_map[balance_osd].primary_pg.size() == 0 
                      || osds_map[balance_osd].primary_pg_num > osds_map[balance_osd].primary_pg.size()));
        if(find_osd_retry > max_osd) {
          break;
        }
        ...
        find_pg_retry++;
      } while (balance_pg == invalid_id && find_pg_retry <= max_osd);
      ...
      do {
        ...
        do {
          balance_osd = (balance_osd+1)%max_osd;
          find_osd_retry++;
        } while (find_osd_retry <= max_osd && (!osds_map.count(balance_osd) || osds_map[balance_osd].duplicate_pg.size() == 0 ||
