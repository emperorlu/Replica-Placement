...
void crush_map::overfull_remap_to_underfull(map<int, set<int>>& change_map,
                                            bool is_primary) {
  while(true) {
    auto over_it = change_map.begin();
    auto under_it = change_map.rbegin();
    if(change_map.empty() || (over_it->first >= 0 && under_it->first <= 1)) {
    ...
    do {
      if(over_it->first < 0) { //overfull
        ...
        do {
          ...
          cout << over_osd << "(" << over_num << ")->" << under_osd<< "(" << under_num <<  ")" << endl;
          if(is_primary) {
            vector<int> to_remove;
            for(int pg : osd_to_primary_pgs[over_osd]) {
              bool collide = false;
            ...
            for(int pg : osd_to_pgs[over_osd]) {
              ...
              if(!collide) {
                ...
                for(int i = 0; i < pgs_to_osd[pg].size(); i++) {
                  if(pgs_to_osd[pg][i] == over_osd) {
                    pgs_to_osd[pg][i] = under_osd;
                    //cout << pg <<" " << i<<  ":" << over_osd << "->" << under_osd << endl;
                    break;
                  }
                }
                over_num++;
      ...
      } else { // only underfull and underfull greater than 1
    ...
    } while(!remaped &&under_it != change_map.rend() && under_it->first > 0);
  }
}
